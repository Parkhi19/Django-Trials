...
def StudentSummary(request,link):
    ...
    if user_type == 's':
        ...
        start_d = str(assignment.start).split('+')[0]
        first_d=str(assignment.first_sub_time).split('+')[0]
        ...
        if oc ==1:
            ...
            return render(request,"student_assignment/summary.html",{"front":res,"score":score,"assign":assignment,"endtime":first_d})
        elif oc==2:
            return render(request,'CreateAssignment/something.html',{'msg':'Assignment  is not started, check the link'})
        elif oc==3:
