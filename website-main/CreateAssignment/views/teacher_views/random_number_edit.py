...
def RandomNumberEdit(request,link,qno):
    ...
    print(request.POST["integer_only"])
    mi,ma,io = request.POST["min_vals"].split(','),request.POST["max_vals"].split(','),request.POST["integer_only"].split(',')
    for i in mi:
        try:
            float(i)
        except:
    ...
    for each in randoms:
        each.min_num = mi[i]
        each.max_num = ma[i]
        each.integers_only = bool(int(io[i]))
        i+=1
        each.save()
