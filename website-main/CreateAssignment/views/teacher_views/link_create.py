...
def LinkCreate(request):
    ...
    if pro is None:
        ...
        if request.method == "POST":
            if request.POST["course_name"].strip() == "" or request.POST["assignment_name"].strip() == ""  or request.POST["start"] =="":
            ...
            second_sub_time = None if request.POST["second_sub_time"] == "" else request.POST["second_sub_time"].replace('T',' ')
            result_time =  None if request.POST["result_time"] == "" else request.POST["result_time"].replace('T',' ')
            
            if first_sub_time and second_sub_time is not None:
            ...
            if first_sub_time and second_sub_time is not None:
                ...
                if str(datetime.datetime.strptime(second_sub_time, '%Y-%m-%d %H:%M')-datetime.datetime.strptime(first_sub_time, '%Y-%m-%d %H:%M'))[0] == '-':
                    ...
                    return redirect('./')
            if result_time and second_sub_time is not None:    
