...
function ck(k) {
  ...
  if(document.getElementById('chance0')){
  ...
  if(document.getElementById('chance3')){
    document.getElementById('chance3').addEventListener('click', function () {
      ck(3);
      checkit = true;
    });
  }
  if(document.getElementById('chance4')){
    document.getElementById('chance4').addEventListener('click', function () {
      ck(4);
      checkit = true;
    });
  }
...
function work(k) {
  ...
  totalTime = "05:00";
  document.getElementById("questionTimeLeft").textContent = totalTime;
  ...
  function loadQuiz() {
    ...
    if (one) {
      ...
      for (let i = 0; i < Object.keys(currentQuizData.options).length; i++) {
        ...
        li.insertAdjacentHTML('afterend', `<p><input type="radio" name="answer" id="${avar}" class="answer" /><label for="${avar}" class="optionbox" ">${Object.values(currentQuizData.options)[i]}</label></p>`);
        charcode++;
      }
    } else if (two) {
      ...
      for (let i = 0; i < Object.keys(currentQuizData.options).length; i++) {
        ...
        ul.appendChild(li);
        let avar = String.fromCharCode(charcode);
        li.insertAdjacentHTML('afterend', `<p><input type="checkbox" name="answer" id="${avar}" class="answermul" /><label for="${avar}" class="optionbox">${Object.values(currentQuizData.options)[i]}</label></p>`);
        charcode++;
      }
    }
  }
...
//ON RELOADING
  ...
  if (performance.navigation.type == 1) {
    let str = new String
    let ans = new String
    for(let i =0 ; i<obj[0].questions.length;i++){
      str+= obj[0].questions[i].number+',';
      ans+= ',';
    }
  document.getElementById('ans').value = ans.substr(0, ans.length - 1);
  document.getElementById('all_questions_in_string').value = str.substr(0, str.length - 1);
